"""Real end-to-end integration test: RolloutGateway against a live vLLM server."""
...
pytestmark = [
    pytest.mark.skipif(not HAVE_DEPS, reason="needs openai + anthropic + vllm (real-model E2E)"),
    pytest.mark.asyncio,
]


# ---------------------------------------------------------------------------
# server lifecycle (session fixture owns the vLLM process)
# ---------------------------------------------------------------------------


def server_up() -> bool:
    try:
        with urllib.request.urlopen(f"{VLLM_URL}/v1/models", timeout=5):
            return True
...
def server():
    """Launch vllm serve unless one is already running at E2E_VLLM_URL."""
    if server_up():
    ...
    with open(SERVER_LOG, "w") as log:
        proc = subprocess.Popen(
            [vllm_bin, "serve", MODEL, "--port", VLLM_URL.rsplit(":", 1)[-1]],
            stdout=log,
            stderr=subprocess.STDOUT,
            start_new_session=True,  # own process group -> clean teardown
        )
    while not server_up():
        assert proc.poll() is None, f"vllm server died; see {SERVER_LOG}"
        time.sleep(3)
    yield
...
# ---------------------------------------------------------------------------
# gateway assembly: real renderer + real vLLM backend (with call recording)
# ---------------------------------------------------------------------------


class RecordingBackend:
    """Delegates to the real VllmHttpBackend, recording each turn so the test
    can make token-exact assertions against what vLLM actually returned."""
...
def make_gateway():
    ...
    backend = RecordingBackend(VllmHttpBackend(VLLM_URL))
    renderer = HfTemplateRenderer(_TOKENIZER)
    gateway = RolloutGateway(backend=backend, renderer=renderer, tokenizer=_TOKENIZER)  # both adapters (default)
...
async def test_single_turn_token_exact(server):
    """One real generation; the TraceRecord must be token-for-token what vLLM saw."""
    ...
    # logprobs are vLLM's real sampling logprobs, passed through untouched
...
async def test_anthropic_client_multi_turn_token_exact(server):
    ...
    with serve(gateway) as base_url:
        ...
        client = anthropic.AsyncAnthropic(base_url=base_url, api_key=sid)
        history = [{"role": "user", "content": "What is 2+2? Reply with just the number. /no_think"}]
        resp1 = await client.messages.create(model=MODEL, messages=history, max_tokens=MAX_NEW_TOKENS)
        assert resp1.role == "assistant" and resp1.stop_reason in ("end_turn", "max_tokens")
        content1 = "".join(b.text for b in resp1.content if b.type == "text")
        assert "4" in content1, f"model answered: {content1!r}"

        history += [
            {"role": "assistant", "content": content1},
            {"role": "user", "content": "Now add 10 to that. Reply with just the number. /no_think"},
        ]
        resp2 = await client.messages.create(model=MODEL, messages=history, max_tokens=MAX_NEW_TOKENS)
        assert any(b.type == "text" and b.text for b in resp2.content)

        records = await gateway.finish_session(sid, base_sample=BaseTrace(rollout_id="e2e-5"), reward=1.0)
...
async def test_mixed_protocol_clients_share_one_trajectory(server):
    ...
    with serve(gateway) as base_url:
        ...
        oai = openai.AsyncOpenAI(base_url=f"{base_url}/v1", api_key=sid)
        history = [{"role": "user", "content": "What is 2+2? Reply with just the number. /no_think"}]
        resp1 = await oai.chat.completions.create(model=MODEL, messages=history, max_tokens=MAX_NEW_TOKENS)
        content1 = resp1.choices[0].message.content
        assert content1

        anth = anthropic.AsyncAnthropic(base_url=base_url, api_key=sid)
        history += [
            {"role": "assistant", "content": content1},
            {"role": "user", "content": "Now add 10 to that. Reply with just the number. /no_think"},
        ]
        resp2 = await anth.messages.create(model=MODEL, messages=history, max_tokens=MAX_NEW_TOKENS)
        assert any(b.type == "text" and b.text for b in resp2.content)

        records = await gateway.finish_session(sid, base_sample=BaseTrace(rollout_id="e2e-6"), reward=1.0)
